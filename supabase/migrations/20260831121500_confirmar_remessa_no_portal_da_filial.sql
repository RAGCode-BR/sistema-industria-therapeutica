-- A confirmação continua visível apenas no portal da filial destinatária.
-- A conta da própria filial confirma diretamente; o administrador do CD pode
-- operar a visualização dessa filial sem alterar o vínculo do pedido.
...
declare
  ...
  v_filial_id text;
  v_situacao text;
  v_papel text := public.meu_papel()::text;
...
begin
  ...
  if v_papel <> 'cd_admin'
     and (v_papel <> 'filial' or v_filial_id <> public.minha_filial_id()) then
  ...
  for v_item in
    select produto_id, quantidade from public.remessa_itens where remessa_id = p_remessa_id
  loop
  ...
  set situacao = 'recebida', recebida_em = v_agora,
      recebida_por = auth.uid(), atualizado_em = v_agora
